...
def home(request):
    ...
    return render(request, "news/home.html", {"featured": qs.filter(is_featured=True).first() or qs.first(), "latest": qs[:9], "breaking": qs.filter(is_breaking=True)[:6]})
...
def category_articles(request, slug):
    qs = Article.objects.filter(status="published", category__slug=slug).select_related("category", "city")
...
def search(request):
    q = request.GET.get("q", "").strip(); qs = Article.objects.filter(status="published")
